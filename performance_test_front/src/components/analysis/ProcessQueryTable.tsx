...
const DEFAULT_PROCESS_WIDTHS = [80, 100, 80, 150, 150, 150, 150, 150]; // 7개 컬럼 가정
...
const ProcessQueryTable: React.FC<{ data: TestEzisProcessQueryHistoryDto[] }> = ({ data }) => {
    ...
    const headers = [
        '분류', '테스트명', '소요 시간(초)', 'User Agent', 'CPU 코어 사용량 ((total_core / (row count * time)) * 10k)', 'User CPU (정규화 * 10k)', 'System CPU (정규화 * 10k)', '테스트 종료 시간'
    ]; 
    ...
    return (
        <table style={{ 
            ...
            <tbody>
                ...
                {data.map((item, index) => (
                    <tr key={index}>
                        <td style={{ width: columnWidths[0] }}>{item.category}</td>
                        <td style={{ width: columnWidths[1] }}>{item.testName}</td>
